...
* **Emptied the Sheet by accident?** It is recoverable, and the order matters: (1) **don't open the app on the phones first** — the old build pulls the emptied Sheet over its own copy; if you must, put the phone in airplane mode and use *All Records → ⬇ Download backup (JSON)*; (2) **Google Sheets → File → Version history → See version history** → pick the version from before the wipe → **Restore this version** (whole file, all five tabs); (3) deploy this version and reload both phones; (4) *⬆ Import backup (JSON)* on one phone if a JSON copy is newer, then press **🔄 Push app data to Sheet** once — the merge adds back only what is missing. Full walkthrough: HOW-IT-WORKS §12.2.

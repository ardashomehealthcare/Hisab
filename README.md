...
* **📥 Add missing records** (*All Records*) — while this file is empty (as now) it asks for a Hisab file on the phone: a *⬇ backup*, or an older copy of this records file. It **reads your Google Sheet first**, shows how many records are missing per tab, asks, and adds **only those**. Nothing is replaced or deleted, nothing the Sheet already has is added twice, and an old copy never goes over a newer one. If the Sheet cannot be read, nothing is added at all. (If this file carries records again, 📥 offers those the same way.)
* **Duplicates are skipped** — an employee is matched by name, everything else by person/client + date + amount — so importing twice cannot double the books. A row deleted on the phone that has not left the Sheet yet is not brought back either.
* **One thing it cannot know:** a record deleted *after* the file was made, whose deletion has already reached the Sheet, comes back when the file is added — delete it again.
* Added records are marked *not sent yet*, so **🔄 Push app data to Sheet** writes them into your own Sheet when you are ready.
* The file is a plain Hisab backup (the same shape as *⬇ Download backup (JSON)*), so you can also edit it, or add it on a phone with **📥 Add missing records**. (*⬆ Import backup (JSON)* is different: it **replaces** everything on the phone, so records changed since go back to the file's version when you push, and entries typed in again are sent twice.)
...
* **Emptied the Sheet by accident? It is recoverable.** (1) **Reload both phones first** (an older cached copy still writes the old way — compare 📱 *App version* on the *Google Sheet* tab); (2) if a phone still shows yesterday's rows, press **🔄 Push app data to Sheet** once — the merge adds back only what the Sheet is missing; (3) if not, on a computer use Google's own **File → Version history → See version history** → pick the version from just before the wipe → **Restore this version** (whole file, all five tabs); (4) from a file — a JSON backup downloaded earlier, or an older copy of `data/hisab-data.json` from the project's history — use *All Records* → **📥 Add missing records** and pick the file: it reads the Sheet first and adds only what the Sheet is missing (nothing doubled, no later change undone), then press **🔄 Push app data to Sheet**. Full walkthrough: HOW-IT-WORKS §12.2.
* **A wipe can no longer spread.** The old build emptied the tabs and wrote that phone's few rows; every other phone then read its own rows as “deleted on another phone” and threw them away. Now a pull that would remove **six rows or more *and* a third of a tab at once** is read as a wipe, not as deletions: the rows are kept, marked *not sent yet*, and an amber notice says so — one press of **🔄 Push app data to Sheet** puts them back. Single deletions still travel between the phones as before (HOW-IT-WORKS §12.3).
...
4. Press **💾 Backup app data to second Sheet** whenever you want a complete copy. This backup is **upload-only**: Hisab clears and replaces the backup tab rows with the current device snapshot; it never imports rows from the backup Sheet into the app. If you want the copy after each main-Sheet write or push, tick **Also update this backup after every successful main-Sheet write or push**. The backup is separate, so a failed backup does not stop or undo the main write.
...
collaborative source of truth. An optional second Sheet (`HISAB_BACKUP_SPREADSHEET_ID`) is an
**upload-only** safety copy: configure it on the **Google Sheet** tab, then press **💾 Backup app
data to second Sheet** whenever required. Hisab never loads this backup Sheet into the app; it
replaces its app-managed rows with the current device snapshot. You can turn on automatic backup
after every successful main-Sheet write or push. Backup failure never rolls back a successful
main-Sheet write. On the main Sheet, **🔄 Push app data to Sheet** brings the device and main Sheet
together row by row (by `id`) and writes only what differs; tabs are never cleared.

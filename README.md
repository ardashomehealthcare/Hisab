...
| **Access blocked: authorization error / `origin_mismatch`** | The address the app is served from is not in the client's *Authorised JavaScript origins*. | Add exactly the origin shown by the setup check (e.g. `https://ardashomehealthcare.github.io`, plus `http://localhost:8000` for testing). `file://` can never sign in. |
| **`403: access_denied` / “Google hasn't verified this app”** | Consent screen is **External + Testing** and this address is not a test user, or a Workspace account was asked to consent to an outside app. | **Internal** for a Workspace domain (then sign in with the work address, not a personal `@gmail.com`), or **External** → OAuth consent screen → **Test users** → add every partner, or publish the app. |

...
| **All Records** | Every saved entry, filter by employee/month, delete entries, download / import JSON backup • **🗓 Saved copies** — the dated copy of the books this phone keeps for itself (once a day, last ten days): put a day's books back, save one as a file, or delete them • **🕘 Bring back a past version** — if the Sheet was emptied, the version Google kept from just before it is read on the phone and the missing rows are merged back (nothing is deleted or replaced), or restore from an **Excel / CSV / JSON** file |
...
* **Emptied the Sheet by accident? It is recoverable — from the phone itself.** (1) **Reload both phones first** (an older cached copy still writes the old way — compare 📱 *App version* on the *Google Sheet* tab); (2) if a phone still shows yesterday's rows, press **🔄 Push app data to Sheet** once — the merge adds back only what the Sheet is missing; (3) if not, *All Records* → **🕘 Bring back a past version** → **🔄 Look up the Sheet’s past versions** → pick the one from just before the wipe → **👁 Read this version** → **✅ Add these rows** → **🔄 Push app data to Sheet**; (4) or use the dated copy this phone keeps — *All Records* → **🗓 Saved copies** → **↩ Put back**; (5) on a computer, Google's own *File → Version history → Restore this version* does the same for the whole file. Every one of these paths only **adds** rows, so today's entries are never thrown away. Full walkthrough: HOW-IT-WORKS §12.2.
* **A wipe can no longer spread.** The old build emptied the tabs and wrote that phone's few rows; every other phone then read its own rows as “deleted on another phone” and threw them away. Now a pull that would remove **six rows or more *and* a third of a tab at once** is read as a wipe, not as deletions: the rows are kept, marked *not sent yet*, and an amber notice offers **🔄 Put my rows back in the Sheet**. Single deletions still travel between the phones as before (HOW-IT-WORKS §12.3).
* **A dated copy is kept on the phone.** `hisabSafetyCopy_v1` holds the five tables once a day for the **last ten days**; a day's copy never shrinks, the oldest fall off by themselves, and *All Records → 🗓 Saved copies* restores from them. A *this device only* clear deliberately leaves them in place, so an accidental clear can be undone.

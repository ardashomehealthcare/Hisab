...
|---|---|
| **Dashboard** | ⏰ **Payment collection reminders** — each active client's collection runs **every 15 days from their duty start**: the card shows *due / overdue / due in X days*, disappears by itself when the payment is recorded in Money Entry, and **✕ Cancel reminder** hides one cycle (it returns next cycle); ended clients are never reminded |
| **Employees** | Name, phone, monthly / per-day wage, wage amount, 24 hr / 12 hr duty — **24 hr: duty start time • 12 hr: ☀️ day or 🌙 night duty** — joining date, client name • each row has a small **⏹ End duty** button (ends only that employee — records stay, ↩ brings them back — and if their client is still active the app offers to open the Add-employee form with client, phone & deal pre-filled so the **replacement** is assigned in one step); **🛑 End client duty** lives in the Client billing calculator and ends the client **together with every active employee posted there** |
